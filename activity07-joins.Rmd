...
**Response**: La Quinta has numerous locations outside the USA in Canada, Mexico, New Zealand, Honduras, Turkey, United Arab Emirates, Chile, and Colombia. On the other hand, Denny's is exclusively located in the USA.
...
**Response**: We would have to filter out columns containing La Quinta locations in the USA.
...
```{r Denny's_outside_usa, message = FALSE}
filter(dennys, !state %in% states$abbreviation)
```



**Response**: There are no locations outside th USA.
...
```{r dannys_outside_usa, message = FALSE}
dennys %>%
...
```{r laquinta_outside_usa, message=FALSE}
laquinta %>%
  anti_join(states, by = c("state" = "abbreviation"))
```
...
```{r fewest_denny's, message=FALSE}
dennys%>%
  group_by(state)%>%
  summarise(count = n())
```
...
```{r fewest_laquinta, message=FALSE}
laquinta_us%>%
  group_by(state)%>%
  summarise(count = n())%>%
  arrange(desc(count))
...
**Response**: No its not, since Alaska would have less restaurants and hotels.
...
```{r locations_per_thousand_miles, message=FALSE}
dennys%>%
  group_by(state)%>%
  summarise(count = n())%>%
  inner_join(states, by = c("state" = "abbreviation"))%>%
  mutate(dennysPTM = (count/area)*1000)%>%
  arrange(desc(dennysPTM)) 
...
```{r Laquinta_per_square_thousand_miles}
laquinta_us %>%
  group_by(state)%>%
  summarise(count = n())%>%
  inner_join(states, by = c("state"="abbreviation"))%>%
  mutate(laquintaPSM = (count/area)*1000)%>%
  arrange(desc(laquintaPSM))
```
...
**Response**:The rows should display individual locations.
...
**Response**:There are fewer Denny's locations in southern states and some parts of the midwestern region. Generally, there are fewer Denny's and La_Quinta locations in the central states.
...
**Response**: There are only 4 LaQuinta locations in Michigan.2 locations are right next to Denny's, so the joke stands.
...
**Response**:it looks like both Denny's and La Quinta occupy almost a similar fraction of locations in Texas.
